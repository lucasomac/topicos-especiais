        ...
        Alert.alert("Atenção", `Tem certeza que deseja excluir " ${props.item.name}"?`,
            [{
            ...
            },
                {
                    text: "Sim",
                    onPress: () => {
                        ProductModel.deleteItem(props.item.id).then(response => props.navigation.navigate("ProductList", {id: props.item.id}));
                    }
                }
            ],
            {
    ...
    return (<View style={styles.container}>
        ...
        <View style={styles.buttonsContainer}>
            <TouchableOpacity style={styles.deleteButton} onPress={handleDeletePress}>
                <MaterialCommunityIcons name="delete-forever" color="#FFF" size={26}/>
            </TouchableOpacity>
            <TouchableOpacity style={styles.editButton} onPress={handleEditButton}>
                <MaterialCommunityIcons name="file=document-edit-outline" color="#FFF" size={26}/>
            </TouchableOpacity>
        </View>
    </View>);
}
...
const styles = StyleSheet.create(
    {
        container: {backgroundColor: '#FEF', marginTop: 15, width: '100%', borderRadius: 10, padding: 5},
        itemLayoutDetail: {flexDirection: "row"},
        itemTextName: {fontSize: 20, fontWeight: "bold", color: '#730'},
        itemTextDetailTitle: {fontSize: 15, fontWeight: "bold", color: '#730'},
        itemTextDetail: {fontSize: 15},
